...
FRAME_STACK = 4
INPUT_SHAPE = (FRAME_STACK, 84, 84) # (C, H, W)
...
TARGET_UPDATE_FREQ = 1000   # Fréquence de mise à jour du target network (en nombre de pas)
...
RND_LR = 1e-4               # Taux d'apprentissage pour le prédicteur RND
INTRINSIC_REWARD_SCALE = 0.1 # Facteur beta pour pondérer la récompense intrinsèque (à tuner!)
RND_OBS_CLIP = 5.0          # Clipping pour les observations normalisées RND
RND_REWARD_CLIP = 5.0       # Clipping pour les récompenses intrinsèques normalisées
...
SAVE_FREQ = 100000          # Fréquence de sauvegarde du modèle (en nombre de pas)
